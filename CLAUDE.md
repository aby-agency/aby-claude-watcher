...
- Cartes grid : rendu sobre voulu — pas de bloom radial ni bordure teintée pleine carte (retirés v1.12.x, « liquid glass » raté) ; l'état passe par le badge (halo léger + dot) et le filet 1px en haut. `--glow` reste dans styles.css : le popover s'en sert pour ses halos
...
- Vue office (v3 « les salles ») : 3 salles PARTAGÉES par fonction — Travail (running/thinking/pending/error, un poste par session, LED + bulle émote + étiquette pixel par perso), Pause (waiting : café, canapés, fontaine/distributeur, zzz), Recherche (subagents + agents de workflows + coin sombre headless). Les persos (stables par projet) MIGRENT entre salles (sortie porte → entrée porte, slots stables par salle, salles qui ne rétrécissent jamais sous leurs occupants réels). 3 canvases fixes, UN timer 8 fps coupé vue inactive ; clic sur un perso = focus session (headless exclu) ; étiquettes anti-collision (bulles prioritaires). Assets jamais commités (licence LimeZu, repo public) : `npm run bake` régénère `ui/office-assets/` (gitignoré) — À FAIRE AVANT tout build DMG.
